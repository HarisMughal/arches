...
9. In settings.py:
    ...
    3. Update `LOCALE_PATHS.append(...)` to `LOCALE_PATHS.insert(0, ...)`
        ...
        ```
    4. Remove the `ARCHES_APPLICATIONS` setting. If your application is an Arches Application (designed for redistribution), you should create an AppConfig in apps.py with the class attribute `is_arches_application = True`.
    5. Replace:
        ```
