...
9. In settings.py:
    ...
    3. Update `LOCALE_PATHS.append(...)` to `LOCALE_PATHS.insert(0, ...)`
        ...
        ```
    4. Remove the `ARCHES_APPLICATIONS` setting.
    5. Replace:
        ```
    ...
    9. Ensure your project is the last entry in `INSTALLED_APPS`.
...
11. Add an `apps.py` in the project root:
    ```
    ...
    class ${project_name}Config(AppConfig):
        ...
        def ready(self):
            ...
            if self.name == settings.INSTALLED_APPS[-1]:
                ...
                # consider adjusting urls for i18n here also (?)
    ```

12. Run `python manage.py updateproject`

13. If you do any custom development, update and reinstall the python development dependencies, and install the pre-commit hooks. (You can always ignore these hooks with `git commit --no-verify`.)
...
14. Navigate to the directory with your project's package.json file. Then run:
    ```
    npm install
    ```

15. Start your application server in a separate terminal if it's not already running.

16.  In the same terminal window where you ran `npm` ( on the same level as `package.json` ):
